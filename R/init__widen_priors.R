...
widen_priors <- function(priors) {
    priors[stringr::str_starts(parameter, "cov_"),
           `:=`(val1 = 0,
                val2 = pmax(val2, ceiling(true_val + 1), na.rm = TRUE))]

    priors[stringr::str_starts(parameter, "r_"),
           `:=`(val1 = -0.95, val2 = 0.95)]

    priors[stringr::str_detect(parameter, "period|beta|sigma"),
           `:=`(val1 = 0,
                val2 = pmax(val2, true_val * 1.2, na.rm = TRUE))]

    priors[stringr::str_ends(parameter, "shape"),
           `:=`(val1 = 0.5, val2 = 5)]

    priors[stringr::str_starts(parameter, "trial|donor|weight"),

#' Title
#'
#' @param popn A pedigree in data.table format
#' @param use_grm A string listing what type of A matrix to produce.
#' - "A" is a standard A matrix
#' - "A_nz" is A in sparse format
#' - "A_inv" is inverse A
#' - "A_inv_nz" is inverse A in sparse format
#'
#' @returns A numerator relationship "A" Matrix
#' @export

make_grm <- function(popn, use_grm) {
    switch(use_grm,
           "A" = make_A_matrix(popn) |>
               with(Matrix::sparseMatrix(i = i, j = j, x = x)),
...
make_A_matrix <- function(popn) {
    ...
    if (make_sparse) {
        ...
        # GRM <- with(tmp[value > 0],
        #             Matrix::sparseMatrix(i = i, j = j, x = value))
    } else {
